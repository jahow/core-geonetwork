	...
	<xsl:template mode="dataset" match="gmd:MD_Metadata">
      ...
      and gmd:applicationProfile/gco:CharacterString = $applicationProfile]/gmd:description">
		...
		<xsl:if test="not($isServiceEntry)">
			...
			<xsl:variable name="crs" select="gmd:referenceSystemInfo/gmd:MD_ReferenceSystem/gmd:referenceSystemIdentifier/gmd:RS_Identifier//gco:CharacterString" />
			<author>
				<name><xsl:value-of select="$authorName"/></name>
				<email><xsl:value-of select="$authorEmail"/></email>
			</author>
			...
			<xsl:choose>
				<xsl:when test="not($isLocal)">
					<!--We never use this variable so dont understand why this test ?????-->
					<xsl:for-each
				...
				<xsl:otherwise>
					...
					<xsl:for-each select="./gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource">
						...
						<xsl:if test="(upper-case($currentProtocol)=$protocolD or upper-case($currentProtocol)=$protocolL) and $currentAppProfile=$applicationProfile">
							<xsl:if test="$requestedCRS='' or $requestedCRS=$crs">
								<xsl:variable name="mimeFileType" select="normalize-space(gmd:name/gmx:MimeFileType)" />
								<entry>
									<inspire_dls:spatial_dataset_identifier_code>
										<xsl:value-of select="$identifierCode" />
									</inspire_dls:spatial_dataset_identifier_code>
									...
									<link title="{./gmd:description/gco:CharacterString/text()}" rel="alternate">
										<xsl:attribute name="type"><xsl:value-of select="$mimeFileType" /></xsl:attribute>
										<xsl:attribute name="href"><xsl:value-of select="./gmd:linkage/gmd:URL" /></xsl:attribute>
										<xsl:variable name="length">
											<xsl:value-of
									...
									<xsl:variable name="crsCode" select="substring-after($crsEpsgCode, 'EPSG:')" />
									<xsl:variable name="crsLabel">
										<xsl:choose>
											<xsl:when test="$crsCode = '2154'">RGF93 / Lambert-93</xsl:when>
											<xsl:when test="$crsCode = '32620'">WGS 84 / UTM zone 20N</xsl:when>
											<xsl:when test="$crsCode = '2972'">RGFG95 / UTM zone 22N</xsl:when>
											<xsl:when test="$crsCode = '2975'">RGR92 / UTM zone 40S</xsl:when>
											<xsl:when test="$crsCode = '4467'">RGSPM06 / UTM zone 21N</xsl:when>
											<xsl:when test="$crsCode = '4467'">RGM04 / UTM zone 38S</xsl:when>
										</xsl:choose>
									</xsl:variable>
									<category term="{concat('http://www.opengis.net/def/crs/EPSG/0/', $crsCode)}" label="{$crsLabel}" />
									<updated><xsl:value-of select="$updated" />Z</updated>
									<!-- <georss:polygon>
									   <xsl:value-of select="concat($fs,' ',$fw,' ',$fn,' ',$fw,' ',$fn,' ',$fe,' ',$fs,' ',$fe,' ',$fs,' ',$fw)" />
									 </georss:polygon> -->
